...
export const config = {
  ...
  splitMode,
  port: parsePort(),
  schema: splitMode ? requireEnv("MEMBER_DB_SCHEMA") : readEnv("MEMBER_DB_SCHEMA") || "public",
  databaseUrl: splitMode ? requirePostgresUrl("MEMBER_DATABASE_URL") : readEnv("MEMBER_DATABASE_URL") || readEnv("DATABASE_URL"),
  supabaseUrl: splitMode ? requireHttpUrl("MEMBER_SUPABASE_URL") : legacySupabaseUrl,
  supabaseServiceKey: splitMode ? requireEnv("MEMBER_SUPABASE_SERVICE_ROLE_KEY") : legacySupabaseServiceKey,
};
