...
export const config = {
  ...
  splitMode,
  port: parsePort(),
  schema: splitMode ? requireEnv("REWARD_DB_SCHEMA") : readEnv("REWARD_DB_SCHEMA") || "public",
  databaseUrl: splitMode ? requirePostgresUrl("REWARD_DATABASE_URL") : readEnv("REWARD_DATABASE_URL") || readEnv("DATABASE_URL"),
  supabaseUrl: splitMode ? requireHttpUrl("REWARD_SUPABASE_URL") : readEnv("SUPABASE_URL"),
  supabaseServiceKey: splitMode
    ? requireEnv("REWARD_SUPABASE_SERVICE_ROLE_KEY")
