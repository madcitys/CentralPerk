...
export default function AdminRewardsPage() {
  ...
  useEffect(() => {
    if (!selectedPartnerId && partners[0]?.id) {
  ...
  const handleSettlePartner = async (partnerId: string) => {
    try {
      ...
      await reload();
      window.open(`/api/partners/${partnerId}/settlement/${settlementMonth}/pdf`, "_blank", "noopener,noreferrer");
          ...
          <Card className={adminPanelClass}>
            ...
            <div className="mt-5 rounded-[28px] border border-[#dbe8f6] bg-[linear-gradient(180deg,#ffffff_0%,#f7fbff_100%)] p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedPartnerRow?.linkedRewards.map((reward) => <Badge key={reward.reward_id} variant="outline">{reward.name}</Badge>)}
