...
export function clearMemberActivityCache(memberId?: string, fallbackEmail?: string) {
  if (!memberId) {
    memberActivityCache.clear();
    return;
  }

  const normalizedMemberId = String(memberId).trim();
  const normalizedEmail = String(fallbackEmail || "").trim();
  memberActivityCache.delete(`${normalizedMemberId}:`);
  memberActivityCache.delete(`${normalizedMemberId}:${normalizedEmail}`);

  for (const key of memberActivityCache.keys()) {
    if (key.startsWith(`${normalizedMemberId}:`) || (normalizedEmail && key.endsWith(`:${normalizedEmail}`))) {
      memberActivityCache.delete(key);
    }
  }
}
...
function parseLimit(value: unknown, fallback = 200) {
  const limit = Number(queryValue(value) || fallback);
  if (!Number.isFinite(limit)) return fallback;
  return Math.min(2000, Math.max(1, Math.floor(limit)));
}

function normalizeServiceMember(row: Record<string, unknown>) {
  const memberNumber = String(row.member_number ?? row.memberNumber ?? row.id ?? "");
  const firstName = String(row.first_name ?? row.firstName ?? "").trim();
  const lastName = String(row.last_name ?? row.lastName ?? "").trim();
  return {
    ...row,
    id: row.memberId ?? row.member_id ?? row.id ?? memberNumber,
    member_id: row.memberId ?? row.member_id ?? row.id ?? memberNumber,
    ...
    points_balance: Number(row.points_balance ?? row.pointsBalance ?? 0),
...
async function loadFastMemberActivity(memberId: string, fallbackEmail?: string): Promise<MemberActivityResponse> {
  ...
  if (useRemoteMicroservices()) {
    await requireGatewayUpstream("points", "Points engine");
  }
...
export const membersListHandler = createApiHandler({
  route: "/api/members",
  ...
  handler: async ({ req }) => {
    ...
    if (useRemoteMicroservices()) {
      ...
      return {
        ok: true as const,
        members: (response.members || []).map(normalizeServiceMember),
        source: response.source ?? "member-service",
      };
    }
    ...
    return {
      ok: true as const,
      members: (data || []).map((row) => normalizeServiceMember(row as Record<string, unknown>)),
      source: "supabase",
    };
  },
});
...
export const memberPointsHandler = createApiHandler({
  ...
  handler: async ({ req }) => {
    ...
    const activity = await loadFastMemberActivity(memberId, fallbackEmail);
    return {
      ok: true as const,
      memberId,
  ...
  handler: async ({ req }) => {
    ...
    if (useRemoteMicroservices()) {
      ...
      return {
        ok: true as const,
        memberId,
        profile: normalizeServiceMember(response.member || {}),
        source: response.source ?? "member-service",
      };
    }
...
export const memberPreferencesHandler = createApiHandler({
  ...
  handler: async ({ body, req }) => {
    ...
    const current = useLocalRuntimeFirst()
      ...
      : await loadCommunicationPreference(memberId, fallbackEmail);
    const preference = {
      ...current,
      ...body,
    };
    if (useLocalRuntimeFirst()) {
      ...
      await saveCommunicationPreference(memberId, preference, fallbackEmail);
    }
    return {
      ok: true as const,
      memberId,
