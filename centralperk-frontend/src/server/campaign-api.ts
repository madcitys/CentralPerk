...
import { requireGatewayUpstream, useRemoteMicroservices } from "./microservice-client";
...
async function requireCampaignService() {
  if (useRemoteMicroservices()) {
    await requireGatewayUpstream("campaign", "Campaign service");
  }
}
...
export const campaignPerformanceHandler = createApiHandler({
  ...
  handler: async () => {
    ...
    await requireCampaignService();
    const response = await loadCampaignPerformance();
...
export const publishCampaignHandler = createApiHandler({
  ...
  handler: async ({ body, req }) => {
    ...
    if (body.queueNotifications && useRemoteMicroservices()) {
    ...
    if (!directPublish?.ok) throw new HttpError(502, "Campaign service publish failed");
    ...
    return {
      ...
      notificationsQueued: directPublish.notificationsQueued ?? notificationsQueued,
    };
  },
});
...
export const campaignBudgetStatusHandler = createApiHandler({
  ...
  handler: async ({ req }) => {
    ...
    await requireCampaignService();
    const directStatus = await loadCampaignBudgetStatus(campaignId);
    if (!directStatus?.ok) throw new HttpError(502, "Campaign service budget status failed");
    return {
      ok: true as const,
