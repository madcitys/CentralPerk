...
async function syncSegmentMembers(segmentId: string, previewMembers: Array<{ id: string }>) {
  const existingRows = await fetchMembersInSegment(segmentId);
  const existingIds = (existingRows as Array<{ member_id?: string | number }>)
    .map((row) => row.member_id)
    .filter((value): value is string | number => value !== undefined && value !== null);

  if (existingIds.length > 0) {
    await removeMembersFromSegment(existingIds, segmentId);
  }
...
export const segmentsHandler = createApiHandler({
  ...
  handler: async ({ body, req }) => {
    if (req.method?.toUpperCase() === "GET") {
      ...
      if (useRemoteMicroservices()) {
        ...
        return {
          ...
          source: response.source ?? "segment-service",
        };
      }
    ...
    await requireGatewayUpstream("segment", "Segment service");
