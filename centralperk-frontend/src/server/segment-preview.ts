...
function isMissingColumnError(error: unknown, column: string) {
  ...
  return (
    message.includes(`column loyalty_members.${column.toLowerCase()} does not exist`) ||
...
export async function previewSegmentAudience(input: {
  ...
  let data: RawMemberRow[] | null = null;
  ...
  if (firstQuery.error) {
    if (!isMissingColumnError(firstQuery.error, "last_activity_at")) throw firstQuery.error;
    ...
    data = (fallbackQuery.data || []) as RawMemberRow[];
  } else {
    data = (firstQuery.data || []) as RawMemberRow[];
  }

  const rows = (data || []).filter((row) => row.id !== undefined && row.member_number);
  const filtered = rows.filter((member) => {
    const results = input.conditions.map((condition) => matchesCondition(member, condition));
    return input.logicMode === "AND" ? results.every(Boolean) : results.some(Boolean);
  });
...
export async function resolveAudienceMembers(input: {
  ...
  const supabase = createServerSupabaseClient();
  let selectColumns = "id,member_number,email,first_name,last_name,tier,points_balance,last_activity_at";
  let query = supabase.from("loyalty_members").select(selectColumns);
  ...
  let data: RawMemberRow[] | null = null;
  const firstQuery = await query.limit(1000);
  if (firstQuery.error) {
    if (!isMissingColumnError(firstQuery.error, "last_activity_at")) throw firstQuery.error;
    selectColumns = "id,member_number,email,first_name,last_name,tier,points_balance";
    let fallbackQuery = supabase.from("loyalty_members").select(selectColumns);
    if (input.memberId) {
      fallbackQuery = fallbackQuery.eq("member_number", input.memberId.trim());
    } else if (input.email) {
      fallbackQuery = fallbackQuery.ilike("email", input.email.trim());
    }
    const fallbackResult = await fallbackQuery.limit(1000);
    if (fallbackResult.error) throw fallbackResult.error;
    data = (fallbackResult.data || []) as RawMemberRow[];
  } else {
    data = (firstQuery.data || []) as RawMemberRow[];
  }

  let rows = (data || []).filter((row) => row.member_number);
  const normalizedSegment = String(input.segment || "").trim().toLowerCase();
