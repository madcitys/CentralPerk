...
import { DEFAULT_TIER_RULES, normalizeTierRules } from "../app/lib/loyalty-engine";
import { fetchTierRulesViaService, PointsServiceError } from "../app/lib/points-service-client";
import { clearMemberActivityCache } from "./member-api";
...
function normalizeRedeemError(error: unknown): never {
  if (error instanceof PointsServiceError) {
    if (error.statusCode === 409 || error.code === "INSUFFICIENT_POINTS") {
...
function normalizeAwardError(error: unknown): never {
  if (error instanceof PointsServiceError) {
    throw new HttpError(error.statusCode, error.message);
  }
  throw error;
...
export const awardPointsHandler = createApiHandler({
  ...
  handler: async ({ req, body }) => {
    ...
    const result = await runWithIdempotency({
      ...
      payload: body,
      execute: async () => {
        const awardResult = await (useLocalRuntimeFirst()
            ? awardLocalPoints(body, idempotencyKey)
            : awardMemberPoints(body, idempotencyKey)
          ).catch(normalizeAwardError);
        clearMemberActivityCache(body.memberIdentifier, body.fallbackEmail);
        return {
          body: {
            ok: true as const,
            result: awardResult,
...
export const redeemPointsHandler = createApiHandler({
  ...
  handler: async ({ body }) => {
    try {
      const redeemResult = useLocalRuntimeFirst()
...
export const pointsTiersHandler = createApiHandler({
  ...
  handler: async () => {
    ...
    const response = await fetchTierRulesViaService().catch(() => {
      throw new HttpError(503, "Points service is unavailable.");
    });
    if (response?.ok && Array.isArray(response.tiers)) {
      return { ok: true as const, tiers: normalizeTierRules(response.tiers), source: "points_service" };
    }
    throw new HttpError(502, "Points service returned an invalid tier response.");
  },
});
    ...
    const result = await runWithIdempotency({
      ...
      execute: async () => {
        const awardResult = await (useLocalRuntimeFirst()
            ...
            : awardMemberPoints(awardPayload, transactionReference)
          ).catch(normalizeAwardError);
        clearMemberActivityCache(body.memberIdentifier, body.fallbackEmail);
        return {
          body: {
