...
import { Award, CheckCircle2, Copy, QrCode, ShoppingBag, Gift, Store, Truck } from "lucide-react";
...
function buildRedemptionCode(memberId: string, reward: Reward, ledgerId?: string | number) {
  const memberSuffix = memberId.replace(/\D/g, "").slice(-6).padStart(6, "0");
  const source = String(ledgerId || reward.rewardCatalogId || reward.id || Date.now()).replace(/[^a-z0-9]/gi, "").slice(-6);
  return `RDM-${memberSuffix}-${source.toUpperCase() || Date.now().toString(36).toUpperCase()}`;
}

function businessCampaignName(name: unknown, type?: string, code?: unknown) {
  const raw = String(name || "").trim();
  const campaignCode = String(code || "").trim().toLowerCase();
  const normalized = raw.toLowerCase();
  if (normalized.includes("api test double") || campaignCode.includes("test-2x")) return "Gold Member Double Points";
  if (normalized.includes("api test flash") || campaignCode.includes("test-flash")) return "Morning Coffee Flash Sale";
  if (raw) return raw;
  return type === "flash_sale" ? "Flash Sale" : "Campaign";
}

function businessCampaignCode(code: unknown, type?: string) {
  const raw = String(code || "").trim();
  const normalized = raw.toLowerCase();
  if (normalized.includes("test-2x")) return "CMP-GOLD-2X";
  if (normalized.includes("test-flash")) return "FLASH-COFFEE-AM";
  return raw || (type === "flash_sale" ? "FLASH-001" : "CMP-001");
}

function businessCampaignDescription(description: unknown, type?: string) {
  const raw = String(description || "").trim();
  const normalized = raw.toLowerCase();
  if (normalized.includes("synthetic") || normalized.includes("automated test")) {
    return type === "flash_sale"
      ? "Limited-time coffee reward drop for active members."
      : "Business promo for loyalty members during selected campaign windows.";
  }
  return raw;
}
...
function normalizeCampaign(campaign: PromotionCampaign): PromotionCampaign {
  return {
    ...campaign,
    campaignCode: businessCampaignCode(campaign.campaignCode, campaign.campaignType),
    campaignName: businessCampaignName(campaign.campaignName, campaign.campaignType, campaign.campaignCode),
    description: businessCampaignDescription(campaign.description, campaign.campaignType),
...
export default function Rewards() {
  ...
  const [redemptionProof, setRedemptionProof] = useState<RedemptionProof | null>(null);
  ...
  const spendPoints = async (
    ...
    const response = await redeemPointsViaApi({
      ...
      promotionCampaignId: isUuidValue(reward?.activeFlashSaleId) ? reward?.activeFlashSaleId || null : null,
    });
  ...
  const confirmRedeem = async () => {
    ...
    try {
      setSaving(true);
      const response = await spendPoints(safeNumber(selectedReward.pointsCost, 0), `${selectedReward.name} Redemption`, "Reward", "redeemed", selectedReward);
      const proofCode = buildRedemptionCode(user.memberId, selectedReward, response.result.ledgerEntry?.id);
      ...
      toast.success("Reward redeemed!", {
        description:
          redemptionMethod === "online"
            ? `${selectedReward.name} will be delivered. Ref: ${proofCode}`
            : `Show validation code ${proofCode} at the counter.`,
      });
      setRedemptionProof({
        code: proofCode,
        rewardName: selectedReward.name,
        method: redemptionMethod,
        points: safeNumber(selectedReward.pointsCost, 0),
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredRewards.length === 0 && (
        <Card className={`${customerPanelSoftClass} border-dashed border-gray-300`}>
          <p className="text-sm text-gray-600">
            No rewards available from the loyalty backend right now. Check the rewards API or active filters.
          </p>
        </Card>
      )}
      ...
      <Dialog open={Boolean(redemptionProof)} onOpenChange={(open) => !open && setRedemptionProof(null)}>
        <DialogContent className="sm:max-w-md !bg-white !text-gray-900 border border-gray-200 shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-500">
              Show this validation code to the counter or delivery staff.
            </DialogDescription>
          </DialogHeader>
          {redemptionProof ? (
            <div className="space-y-4 py-4">
              <div className="rounded-2xl border border-[#dbe8f6] bg-[#f8fbff] p-4">
                <div className="mb-3 flex items-center gap-3">
                  <div className="rounded-xl bg-white p-3 text-[#1A2B47] shadow-sm">
                    <QrCode className="h-8 w-8" />
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Validation reference</p>
                    <p className="font-mono text-xl font-bold tracking-wide text-[#10213a]">{redemptionProof.code}</p>
                  </div>
                </div>
                <div className="grid gap-2 text-sm text-gray-700">
                  <p><span className="font-semibold text-gray-900">Reward:</span> {redemptionProof.rewardName}</p>
                  <p><span className="font-semibold text-gray-900">Method:</span> {redemptionProof.method === "online" ? "Delivery" : "In-store pickup"}</p>
                  <p><span className="font-semibold text-gray-900">Points used:</span> {redemptionProof.points.toLocaleString()}</p>
                  <p><span className="font-semibold text-gray-900">Redeemed:</span> {new Date(redemptionProof.redeemedAt).toLocaleString()}</p>
                </div>
              </div>
              <p className="text-xs leading-5 text-gray-500">
                Staff can match this code against the redemption transaction in the member history.
              </p>
            </div>
          ) : null}
          <DialogFooter className="gap-2 sm:gap-3">
            <Button
              ...
              className="h-10 border-gray-300 bg-white text-gray-800 hover:bg-gray-50"
              onClick={() => {
                if (!redemptionProof?.code || typeof navigator === "undefined" || !navigator.clipboard) return;
                void navigator.clipboard.writeText(redemptionProof.code).then(() => toast.success("Validation code copied."));
              }}
            >
              <Copy className="mr-2 h-4 w-4" />
              Copy Code
            </Button>
            <Button className={`h-10 ${brandNavySolidClass} ${brandNavySolidHoverClass}`} onClick={() => setRedemptionProof(null)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
