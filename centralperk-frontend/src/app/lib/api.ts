...
import type { ReferralRecord } from "./member-lifecycle";
...
function mapRewardFromApi(row: Record<string, unknown>): Reward {
  const partnerIdValue = row.partner_id;
  return {
    id: String(row.id ?? row.reward_id ?? row.reward_catalog_id ?? ""),
    rewardCatalogId: String(row.reward_catalog_id ?? row.reward_id ?? row.id ?? ""),
    ...
    available: Boolean(row.available ?? row.is_active ?? row.active ?? true),
    expiryDate: row.expiryDate ? String(row.expiryDate) : row.expiry_date ? String(row.expiry_date) : undefined,
    partnerId: typeof partnerIdValue === "string" || typeof partnerIdValue === "number" ? partnerIdValue : null,
    partnerName: row.partner_name ? String(row.partner_name) : null,
    partnerCode: row.partner_code ? String(row.partner_code) : null,
    partnerLogoUrl: row.partner_logo_url ? String(row.partner_logo_url) : null,
    partnerConversionRate: Number(row.partner_conversion_rate ?? 0) || null,
    cashValue: Number(row.cash_value ?? 0) || null,
    activeFlashSaleId: row.active_flash_sale_id ? String(row.active_flash_sale_id) : null,
    flashSaleStartsAt: row.flash_sale_starts_at ? String(row.flash_sale_starts_at) : null,
    flashSaleEndsAt: row.flash_sale_ends_at ? String(row.flash_sale_ends_at) : null,
    flashSaleQuantityLimit: Number(row.flash_sale_quantity_limit ?? 0) || null,
    flashSaleClaimedCount: Number(row.flash_sale_claimed_count ?? 0),
    flashSaleBanner: row.flash_sale_banner ? String(row.flash_sale_banner) : null,
    flashSaleCountdownLabel: row.flash_sale_countdown_label ? String(row.flash_sale_countdown_label) : null,
  };
}

export async function loadRewardsCatalogViaApi(input?: {
  category?: Reward["category"] | "all";
  partnerOnly?: boolean;
  search?: string;
}) {
  ...
  if (input?.partnerOnly) params.set("partnerOnly", "true");
  ...
  return withApiLabel(
    ...
    }>(`/rewards${params.toString() ? `?${params.toString()}` : ""}`),
    "rewards API",
  ).then((response) => (response.rewards || []).map(mapRewardFromApi));
}
...
export async function loadReferralsViaApi(memberId: string) {
  return withApiLabel(
    requestJson<{ ok: true; referrals: ReferralRecord[] }>(
      `/referrals?memberId=${encodeURIComponent(memberId)}`,
    ),
    "referrals API",
  ).then((response) => response.referrals || []);
}
