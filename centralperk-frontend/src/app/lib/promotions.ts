...
function businessCampaignName(name: unknown, type?: string, code?: unknown) {
  ...
  if (normalized.includes("local dev 2x")) return "Local Double Points";
  if (raw) return raw;
...
function normalizeCampaign(row: AnyRecord): PromotionCampaign {
  ...
  const campaignType = String(row.campaignType ?? row.campaign_type ?? "bonus_points") as PromotionCampaignType;
  const campaignCode = businessCampaignCode(row.campaignCode ?? row.campaign_code, campaignType);

  return {
    id: String(row.id ?? ""),
    campaignCode,
    campaignName: businessCampaignName(row.campaignName ?? row.campaign_name, campaignType, campaignCode),
...
export async function loadCampaignPerformance(): Promise<CampaignPerformance[]> {
  return withPromotionReadCache("campaign-performance", async () => {
    ...
    return ((response.performance || []) as AnyRecord[]).map((row) => ({
      ...
      campaignCode: businessCampaignCode(row.campaign_code, row.campaign_type),
      campaignName: businessCampaignName(row.campaign_name, row.campaign_type, row.campaign_code),
