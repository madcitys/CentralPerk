...
export default function AdminRewardsPage() {
  ...
  useEffect(() => {
    ...
    refreshBudgetStatus();
    const interval = window.setInterval(refreshBudgetStatus, 60_000);
  ...
  const prepareFlashSaleCampaign = () => {
    const linkedReward = rewardsCatalog.find((reward) => ["beverage", "food", "voucher"].includes(String(reward.category || "").toLowerCase()));
    const sequence = String(Math.max(1, flashSales.length + 1)).padStart(3, "0");
    setCampaignForm((prev) => ({
      ...prev,
      campaignCode: `FLASH-${sequence}`,
      campaignName: "Morning Coffee Flash Sale",
      description: "Limited-time drink reward drop for active members.",
      ...
      eligibleTiers: "Bronze,Silver,Gold",
      rewardId: linkedReward ? String(linkedReward.id ?? linkedReward.reward_id ?? "") : "",
      flashSaleQuantityLimit: "100",
      bannerTitle: "Morning Coffee Drop",
      bannerMessage: "Redeem the featured drink before the timer ends.",
      countdownLabel: "Flash sale ends in",
      pushNotificationEnabled: true,
    }));
    setCampaignWizardStep(1);
    setActiveTab("campaigns");
    toast.info("Flash sale template loaded. Review it, save, then publish.");
  };
  ...
  const handleSettlePartner = async (partnerId: string) => {
    try {
      ...
      await triggerPartnerSettlementViaApi(partnerId, settlementMonth);
      await reload();
      window.open(apiUrl(`/partners/${partnerId}/settlement/${settlementMonth}/pdf`), "_blank", "noopener,noreferrer");
          ...
          <Card className={adminPanelClass}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">Track active drops or create a new flash sale with a linked reward.</p>
              </div>
              <Button type="button" className={adminDarkButtonClass} onClick={prepareFlashSaleCampaign}>
                Create Flash Sale
              </Button>
            </div>
