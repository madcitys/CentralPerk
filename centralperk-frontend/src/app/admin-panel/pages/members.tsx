  ...
  return (
    <div className={adminPageShellClass}>
      ...
      <div className={adminPanelClass}>
        ...
        <div className="overflow-x-auto rounded-2xl border border-[#e2ebf8] bg-white">
          <table className="w-full min-w-[980px] table-fixed text-sm">
            <thead>
              <tr className="border-b-2 border-gray-200">
                <th className="w-14 py-3 pl-4 pr-2 text-left text-xs font-semibold text-gray-600">Select</th>
                <th className="w-28 px-3 py-3 text-left text-xs font-semibold text-gray-600">Member #</th>
                <th className="w-32 px-3 py-3 text-left text-xs font-semibold text-gray-600">Name</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">Email</th>
                <th className="w-36 px-3 py-3 text-left text-xs font-semibold text-gray-600">Mobile</th>
                <th className="w-24 px-3 py-3 text-left text-xs font-semibold text-gray-600">Points</th>
                <th className="w-44 px-3 py-3 text-left text-xs font-semibold text-gray-600">Segment</th>
                <th className="w-36 px-3 py-3 text-left text-xs font-semibold text-gray-600">Actions</th>
              </tr>
            </thead>
            <tbody>
              {visibleMembers.map((member) => {
                ...
                return (
                  <tr key={key} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-4 pl-4 pr-2 text-gray-700">
                      <input
                        type="checkbox"
                    ...
                    <td className="px-3 py-4 font-medium text-gray-800">{member.member_number}</td>
                    <td className="px-3 py-4 text-gray-700"><span className="line-clamp-2">{member.first_name} {member.last_name}</span></td>
                    <td className="px-3 py-4 text-gray-600" title={member.email || ""}><span className="block truncate">{member.email}</span></td>
                    <td className="px-3 py-4 text-gray-600" title={member.phone || "-"}><span className="block truncate">{member.phone || "-"}</span></td>
                    <td className="px-3 py-4 font-semibold text-gray-800">{(member.points_balance || 0).toLocaleString()}</td>
                    <td className="px-3 py-4 text-gray-600">
                      <select
                        value={member.segment}
                        onChange={(e) => handleMemberSegmentChange(member.member_number, e.target.value)}
                        className={`${adminSelectClass} w-full min-w-0`}
                      >
                    ...
                    <td className="px-3 py-4">
                      <div className="flex flex-col gap-2">
                        <button type="button" onClick={() => setSelectedMember(member)} className="inline-flex w-full items-center justify-center rounded-lg border border-[#c9d8eb] bg-white px-3 py-1.5 text-xs font-semibold text-[#1A2B47] transition hover:border-[#9eb8da] hover:bg-[#eef5ff] hover:text-[#10213a]">View</button>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="inline-flex w-full items-center justify-center rounded-lg bg-[#0fa7b4] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#0c96a2] hover:text-white disabled:opacity-60"
                        >
