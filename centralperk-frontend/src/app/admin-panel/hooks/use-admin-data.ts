...
function isInternalSmokeMember(member: Member) {
  const email = String(member.email || "").toLowerCase();
  const memberNumber = String(member.member_number || "").toLowerCase();
  const name = `${String(member.first_name || "")} ${String(member.last_name || "")}`.trim().toLowerCase();
...
export function useAdminData(options?: { includeInsights?: boolean; scope?: AdminDataScope }) {
  ...
  const fetchData = useCallback(async (options?: { force?: boolean }) => {
    try {
      ...
      const membersWithSegments = ((membersRes.data || []) as Member[]).filter((member) => !isInternalSmokeMember(member)).map((member) => {
