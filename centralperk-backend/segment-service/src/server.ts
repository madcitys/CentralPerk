...
let previewMembersCache: { loadedAt: number; rows: MemberRow[] } | null = null;
...
function isInternalSmokeMember(row: MemberRow) {
...
async function loadPreviewMembers() {
  if (useLocalFallback()) return localMembers;
  ...
  try {
    ...
    const filteredRows = rows.filter((row) => !isInternalSmokeMember(row));
    previewMembersCache = { loadedAt: Date.now(), rows: filteredRows };
    return filteredRows;
  } catch (error) {
    if (!isMissingColumnError(error, "last_activity_at")) throw error;
    const rows = await supabaseRest<MemberRow[]>(
      "loyalty_members?select=id,member_number,first_name,last_name,email,tier,points_balance&limit=1000",
    );
    const filteredRows = rows.filter((row) => !isInternalSmokeMember(row));
    previewMembersCache = { loadedAt: Date.now(), rows: filteredRows };
    return filteredRows;
