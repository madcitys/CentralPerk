...
foreach ($service in $services) {
  ...
  if (Test-Path -LiteralPath $configPath) {
    ...
    foreach ($suffix in @("SCM_FRONTEND_SUPABASE_URL", "SCM_FRONTEND_SUPABASE_SERVICE_ROLE_KEY", "DATABASE_URL", "DB_SCHEMA")) {
    ...
    if ($configText -notmatch "SCM_FRONTEND_USE_SPLIT_SERVICE_DATABASES") {
      Add-Failure "$($service.Name) config does not check SCM_FRONTEND_USE_SPLIT_SERVICE_DATABASES"
    }
...
if (Test-Path -LiteralPath $gatewayDir) {
  Get-ChildItem -LiteralPath $gatewayDir -Recurse -File -Include "*.ts" |
    ...
    ForEach-Object {
      ...
      if ($text -match "(DATABASE_URL|SCM_FRONTEND_SUPABASE_SERVICE_ROLE_KEY|SCM_FRONTEND_SUPABASE_URL)") {
...
Get-ChildItem -LiteralPath $backendRoot -Recurse -File |
  ...
  ForEach-Object {
    $relative = $_.FullName.Substring($backendRoot.Path.Length + 1)
    $text = Get-Content -LiteralPath $_.FullName -Raw
    if ($text -match "process\.env\.(SCM_FRONTEND_SUPABASE_URL|SCM_FRONTEND_SUPABASE_SERVICE_ROLE_KEY|SUPABASE_SECRET_KEY|DATABASE_URL)") {
...
if (Test-Path -LiteralPath $frontendRoot) {
  Get-ChildItem -LiteralPath $frontendRoot -Recurse -File |
    ...
    ForEach-Object {
      ...
      $text = Get-Content -LiteralPath $_.FullName -Raw
      if ($text -match "(SERVICE_ROLE|SUPABASE_SECRET_KEY|SCM_FRONTEND_SUPABASE_SERVICE_ROLE_KEY|DATABASE_URL|SCM_MEMBER_DATABASE_URL|SCM_SEGMENT_DATABASE_URL|SCM_CAMPAIGN_DATABASE_URL|SCM_NOTIFICATION_DATABASE_URL|SCM_REWARD_DATABASE_URL|SCM_POINTS_DATABASE_URL)") {
