import { campaignDb, campaignSupabase, memberDb } from "./supabase-client.js";
import { config } from "./config.js";
import { randomUUID } from "crypto";
const useMemory = config.useLocalFallback ||
    !config.campaignSupabaseUrl ||
    !config.campaignSupabaseServiceKey ||
    !config.memberSupabaseUrl ||
    !config.memberSupabaseServiceKey ||
    config.campaignSupabaseUrl.startsWith("http://127.0.0.1") ||
    config.campaignSupabaseUrl.startsWith("http://localhost");
function buildDefaultCampaigns() {
    return [
        {
            id: randomUUID(),
            campaign_code: "CMP-WELLNESS-2X",
            campaign_name: "Wellness Weekend Double Points",
            description: "Double wellness points on qualified pharmacy and wellness purchases during the weekend.",
...
function stableMemberId(memberIdentifier, fallbackEmail) {
...
function businessCampaignName(name, type, code) {
    const raw = String(name || "").trim();
    const campaignCode = String(code || "").trim().toLowerCase();
    const normalized = raw.toLowerCase();
    const looksSynthetic = normalized.includes("api test") ||
        normalized.includes("synthetic") ||
        normalized.includes("automated test") ||
        normalized.includes("probe") ||
        normalized.includes("codex") ||
        normalized.includes("demo") ||
        normalized.includes("qa ") ||
        normalized.startsWith("qa-") ||
        campaignCode.includes("test-2x") ||
        campaignCode.includes("test-flash") ||
        campaignCode.includes("probe") ||
        campaignCode.includes("codex") ||
        campaignCode.includes("demo") ||
        campaignCode.includes("qa-");
    if (looksSynthetic) {
        return type === "flash_sale" ? "Pharmacy Essentials Flash Sale" : "Gold Wellness Bonus";
    }
    if (normalized.includes("local dev 2x"))
        return "Wellness Weekend Double Points";
    if (raw)
        return raw;
    return type === "flash_sale" ? "Flash Sale" : "Campaign";
}
function businessCampaignCode(code, type) {
    const raw = String(code || "").trim();
    const normalized = raw.toLowerCase();
    if (normalized.includes("test-2x") ||
        normalized.includes("probe") ||
        normalized.includes("codex") ||
        normalized.includes("demo") ||
        normalized.includes("qa-")) {
        return type === "flash_sale" ? "FLASH-PHARM-AM" : "CMP-GOLD-WELLNESS";
    }
    if (normalized.includes("test-flash"))
        return "FLASH-PHARM-AM";
    return raw || (type === "flash_sale" ? "FLASH-001" : "CMP-001");
}
function businessCampaignDescription(description, type) {
    const raw = String(description || "").trim();
    const normalized = raw.toLowerCase();
    if (normalized.includes("synthetic") ||
        normalized.includes("automated test") ||
        normalized.includes("local multiplier") ||
        normalized.includes("probe") ||
        normalized.includes("codex") ||
        normalized.includes("qa ") ||
        normalized.includes("demo")) {
        return type === "flash_sale"
            ? "Limited-time pharmacy reward drop for active members."
            : "Wellness loyalty promotion for qualified pharmacy purchases during the campaign window.";
    }
    return raw || null;
}
function mapCampaign(row) {
    ...
    return {
        id: String(row.id ?? ""),
        campaignCode: businessCampaignCode(row.campaign_code, campaignType),
        campaignName: businessCampaignName(row.campaign_name, campaignType, row.campaign_code),
        description: businessCampaignDescription(row.description, campaignType),
        campaignType,
        status: row.status,
        ...
        flashSaleQuantityLimit: row.flash_sale_quantity_limit === null || row.flash_sale_quantity_limit === undefined
            ? null
...
function buildMemoryRecord(input, existing) {
    return {
        ...
        reward_id: input.rewardId === undefined || input.rewardId === null || input.rewardId === "" ? null : Number(input.rewardId),
        flash_sale_quantity_limit: input.flashSaleQuantityLimit === undefined ? existing?.flash_sale_quantity_limit ?? null : input.flashSaleQuantityLimit,
        flash_sale_claimed_count: existing?.flash_sale_claimed_count ?? 0,
        starts_at: input.startsAt,
        ends_at: input.endsAt,
        budget_limit: input.budgetLimit === undefined ? existing?.budget_limit ?? null : input.budgetLimit,
...
function upsertMemoryCampaign(input) {
    const existingIndex = memory.campaigns.findIndex((campaign) => campaign.id === input.id || campaign.campaign_code === input.campaignCode);
    const existing = existingIndex >= 0 ? memory.campaigns[existingIndex] : undefined;
    const record = buildMemoryRecord(input, existing);
    if (existingIndex >= 0)
        memory.campaigns[existingIndex] = record;
    else
        memory.campaigns.push(record);
    return mapCampaign(record);
}
function memoryCampaign(campaignId) {
    return memory.campaigns.find((campaign) => String(campaign.id) === campaignId);
}
function eligibleForMultiplier(campaign, input) {
    const now = Date.now();
    if (campaign.status !== "active")
        return false;
    if (new Date(campaign.starts_at).getTime() > now)
        return false;
    if (new Date(campaign.ends_at).getTime() < now)
        return false;
    if (campaign.campaign_type !== "multiplier_event")
        return false;
    if (campaign.minimum_purchase_amount > Math.max(0, input.amountSpent))
        return false;
    if (campaign.budget_limit !== null && campaign.budget_spent >= campaign.budget_limit)
        return false;
    if (campaign.eligible_tiers.length > 0 && input.tier) {
        const normalizedTier = input.tier.toLowerCase();
        if (!campaign.eligible_tiers.some((tier) => tier.toLowerCase() === normalizedTier))
            return false;
    }
    return true;
}
function buildPerformanceRow(campaign) {
    return {
        campaign_id: campaign.id,
        campaign_code: businessCampaignCode(campaign.campaign_code, campaign.campaign_type),
        campaign_name: businessCampaignName(campaign.campaign_name, campaign.campaign_type, campaign.campaign_code),
        ...
        sell_through: campaign.flash_sale_quantity_limit && campaign.flash_sale_quantity_limit > 0
            ? Number(((campaign.flash_sale_claimed_count / campaign.flash_sale_quantity_limit) * 100).toFixed(1))
            : null,
    };
}
function buildBudgetStatus(campaign, performance) {
    ...
    const quantityClaimed = Number(performance?.quantity_claimed ?? performance?.quantityClaimed ?? campaign.flashSaleClaimedCount ?? 0);
...
export function seedMemoryCampaign(input) {
    return upsertMemoryCampaign(input);
}
export function getMemoryCampaign(campaignId) {
...
export async function upsertCampaign(input) {
    ...
    const payload = {
        ...
        reward_id: input.rewardId === undefined || input.rewardId === null || input.rewardId === "" ? null : Number(input.rewardId),
        flash_sale_quantity_limit: input.flashSaleQuantityLimit === undefined || input.flashSaleQuantityLimit === null
            ? null
            : Math.max(1, Math.floor(Number(input.flashSaleQuantityLimit))),
        starts_at: input.startsAt,
        ends_at: input.endsAt,
        budget_limit: input.budgetLimit === undefined || input.budgetLimit === null ? null : Math.max(0, Number(input.budgetLimit)),
        auto_pause: input.autoPause ?? true,
    };
    const query = input.id
        ? campaignDb.from("promotion_campaigns").update(payload).eq("id", input.id).select("*").single()
        : campaignDb.from("promotion_campaigns").insert(payload).select("*").single();
...
export async function listCampaigns() {
    if (useMemory)
        return memory.campaigns.map(mapCampaign);
    const { data, error } = await campaignDb.from("promotion_campaigns").select("*").order("starts_at", { ascending: false });
    if (error)
        return memory.campaigns.map(mapCampaign);
...
export async function getCampaignById(campaignId) {
    ...
    if (useMemory)
        return local ? mapCampaign(local) : null;
    const { data, error } = await campaignDb
...
export async function getActiveCampaigns() {
    ...
    const { data, error } = await campaignSupabase.rpc("campaign_active_list");
    if (error)
        return memory.campaigns.map(mapCampaign).filter((campaign) => campaign.status === "active");
    return (data || []).map(mapCampaign);
}
export async function assignVariant(campaignId, memberId) {
    if (useMemory) {
    ...
    const { data, error } = await campaignSupabase.rpc("campaign_assign_variant", {
...
export async function lookupMultiplier(input) {
    if (useMemory) {
    ...
    const { data, error } = await campaignSupabase.rpc("campaign_active_multiplier", {
...
export async function trackBudgetConsumption(campaignId, bonusPoints) {
    if (useMemory) {
        ...
        const paused = Boolean(campaign.auto_pause) &&
    ...
    const { data, error } = await campaignSupabase.rpc("campaign_consume_budget", {
        p_campaign_id: campaignId,
...
export async function findMemberId(memberIdentifier, fallbackEmail) {
    if (useMemory)
        return stableMemberId(memberIdentifier, fallbackEmail);
    const trimmed = memberIdentifier.trim();
    const byNumber = await memberDb
        .from("loyalty_members")
        .select("id")
        .eq("member_number", trimmed)
        .limit(1)
        .maybeSingle();
    if (byNumber.data?.id !== undefined)
        return Number(byNumber.data.id);
    if (fallbackEmail) {
        const byEmail = await memberDb
            .from("loyalty_members")
            .select("id")
            .ilike("email", fallbackEmail)
            .limit(1)
            .maybeSingle();
        if (byEmail.data?.id !== undefined)
...
export async function loadCampaignPerformance() {
    if (useMemory)
        return memory.campaigns.map((campaign) => buildPerformanceRow(campaign));
    const { data, error } = await campaignSupabase.rpc("loyalty_campaign_performance");
    if (error)
        return memory.campaigns.map((campaign) => buildPerformanceRow(campaign));
    return (data || []).map((row) => ({
        ...row,
        campaign_code: businessCampaignCode(row.campaign_code, row.campaign_type),
        campaign_name: businessCampaignName(row.campaign_name, row.campaign_type, row.campaign_code),
    }));
}
export async function getCampaignBudgetStatus(campaignId) {
    const campaign = (await listCampaigns()).find((item) => item.id === campaignId);
    if (!campaign)
        throw new Error("Campaign not found.");
    const performance = (await loadCampaignPerformance()).find((row) => String(row?.campaign_id ?? row?.campaignId ?? "") === campaignId);
...
export async function queueCampaignNotifications(campaignId) {
    ...
    const { data, error } = await campaignSupabase.rpc("loyalty_queue_campaign_notifications", {
        p_campaign_id: campaignId,
    });
    if (error)
        return 0;
...
export async function declareWinner(campaignId, scores) {
    const winner = Number(scores.B) > Number(scores.A) ? "B" : "A";
