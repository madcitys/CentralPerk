...
import { assignMemberVariant, declareCampaignWinner, getBudgetStatus, getActive, getCampaign, getCampaigns, lookupActiveMultiplier, pauseCampaign, queueCampaignNotifications, loadPerformance, publishCampaign, saveCampaign, } from "./engine.js";
export function createServer() {
    ...
    function hasUnresolvedVariable(value) {
    ...
    fastify.post("/campaigns", async (request) => {
        ...
        const campaignCode = parsed.campaignCode?.trim() || `CMP-${randomUUID().slice(0, 8).toUpperCase()}`;
        const campaign = await saveCampaign({ ...parsed, campaignCode });
