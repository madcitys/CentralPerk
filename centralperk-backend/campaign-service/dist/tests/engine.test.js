...
import { assignMemberVariant, declareCampaignWinner, getActive, getBudgetStatus, getCampaign, getCampaigns, loadPerformance, lookupActiveMultiplier, pauseCampaign, publishCampaign, queueCampaignNotifications, saveCampaign, setRepo, } from "../engine.js";
describe("campaign-service unit coverage", () => {
    it("looks up a multiplier and tracks budget consumption", async () => {
        ...
        setRepo({
            ...
            async trackBudgetConsumption(_campaignId, bonusPoints) {
            ...
            async assignVariant() {
                return { campaignId: "cmp-1", memberId: 1, variant: "A" };
            },
            ...
            async declareWinner(_campaignId, scores) {
                return {
                    campaignId: "cmp-1",
    ...
    it("assigns variants evenly for known members", async () => {
        setRepo({
            async findMemberId(memberIdentifier) {
                const numeric = Number(memberIdentifier.replace(/\D+/g, "")) || 1;
                return numeric;
            },
            async assignVariant(campaignId, memberId) {
                return { campaignId, memberId, variant: memberId % 2 === 0 ? "A" : "B" };
            },
        });
        const assignments = await Promise.all(Array.from({ length: 10 }, (_, index) => assignMemberVariant("cmp-42", `member-${index + 1}`)));
        const counts = assignments.reduce((result, assignment) => {
            result[assignment.variant] += 1;
            return result;
        }, { A: 0, B: 0 });
