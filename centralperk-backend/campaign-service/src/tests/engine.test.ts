...
describe("campaign-service unit coverage", () => {
  ...
  it("assigns variants evenly for known members", async () => {
    setRepo({
      async findMemberId(memberIdentifier: string) {
        const numeric = Number(memberIdentifier.replace(/\D+/g, "")) || 1;
        return numeric;
      },
      async assignVariant(campaignId: string, memberId: number) {
        return { campaignId, memberId, variant: memberId % 2 === 0 ? "A" : "B" as const };
      },
    } as any);

    const assignments = await Promise.all(
      Array.from({ length: 10 }, (_, index) => assignMemberVariant("cmp-42", `member-${index + 1}`)),
    );
