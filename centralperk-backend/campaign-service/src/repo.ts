  ...
  return [
    {
      ...
      campaign_name: "Weekend Coffee Double Points",
      description: "Double points for qualified coffee purchases during the weekend.",
...
function businessCampaignName(name: unknown, type?: string, code?: unknown) {
  ...
  if (normalized.includes("local dev 2x")) return "Weekend Coffee Double Points";
  if (raw) return raw;
...
function businessCampaignDescription(description: unknown, type?: string) {
  ...
  const normalized = raw.toLowerCase();
  if (normalized.includes("synthetic") || normalized.includes("automated test") || normalized.includes("local multiplier")) {
