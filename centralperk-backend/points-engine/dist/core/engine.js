...
export async function awardPoints(repo, input) {
    ...
    let pointsToAdd = input.transactionType === "PURCHASE" && input.amountSpent !== undefined
...
export async function redeemPoints(repo, input) {
    const [member, rules] = await Promise.all([
        repo.findMember(input.memberIdentifier, input.fallbackEmail),
        repo.fetchTierRules(),
    ]);
    ...
    console.info("[points-engine] redeem candidate", {
        memberIdentifier: input.memberIdentifier,
        fallbackEmail: input.fallbackEmail,
        pointsToDeduct,
        member,
    });
    if (pointsToDeduct > member.points_balance)
...
export async function runExpiry(repo) {
