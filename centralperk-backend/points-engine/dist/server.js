...
import { loadMemberHistory } from "./supabase-repo.js";
...
function useMemoryPrimary() {
    return (config.useLocalFallback ||
        !config.memberSupabaseUrl ||
        !config.memberSupabaseServiceKey ||
        !config.pointsSupabaseUrl ||
        !config.pointsSupabaseServiceKey ||
...
export function createServer() {
    ...
    fastify.setErrorHandler((error, _request, reply) => {
        ...
        if (lowerMessage.includes("not enough points") || lowerMessage.includes("insufficient")) {
            reply.code(422).send({ ok: false, code: "INSUFFICIENT_POINTS", error: "Insufficient points balance." });
            return;
        }
        ...
        if (statusCode === 409) {
            reply.code(409).send({ ok: false, code: error.code || "CONFLICT", error: message });
            return;
        }
    ...
    const redeemSchema = z.object({
        ...
        receiptId: z.string().trim().max(160).optional(),
    });
    fastify.post("/points/award", async (request) => {
        ...
        const result = await awardPoints(repo, parsed).catch((error) => {
            request.log.error({ err: error, parsed }, "Award failed before fallback.");
            if (repo === memoryRepo || !canUseLocalFallback(error))
                throw error;
    ...
    fastify.post("/points/redeem", async (request) => {
        ...
        const result = await redeemPoints(repo, normalized).catch((error) => {
            request.log.error({ err: error, normalized }, "Redeem failed before fallback.");
            if (repo === memoryRepo || !canUseLocalFallback(error))
                throw error;
    ...
    fastify.get("/points/history", async (request) => {
        const query = request.query;
        const memberIdentifier = String(query.memberIdentifier || "").trim();
        ...
        if (useMemoryPrimary()) {
            const member = await memoryRepo.findMember(memberIdentifier || fallbackEmail || "", fallbackEmail);
            return { ok: true, member, transactions: [] };
        }
        const payload = await loadMemberHistory(memberIdentifier || fallbackEmail || "", fallbackEmail);
        return {
            ok: true,
