...
function makeRepo(initialBalance = 0) {
    ...
    let lastExpiry = { membersProcessed: 0, pointsExpired: 0 };
    const repo = {
        async findMember() {
            return member;
        },
        ...
        async insertAward(_member, input, newBalance, newTier) {
            member = { ...member, points_balance: newBalance, tier: newTier };
...
describe("points-engine unit coverage", () => {
    beforeEach(() => {
        // Reserved for future shared state resets.
    });
    it("normalizes tier rules and preserves a Bronze fallback", () => {
        ...
        assert.deepEqual(normalized.map((rule) => `${rule.tier_label}:${rule.min_points}`), ["Gold:750", "Silver:250", "Bronze:0"]);
    });
    it("calculates purchase points at 1 point per 50 pesos", () => {
        assert.equal(calculatePurchasePoints(123.99), 2);
        assert.equal(calculatePurchasePoints(50), 1);
    ...
    it("blocks redemption when the balance is too low", async () => {
        ...
        await assert.rejects(redeemPoints(repo, {
            memberIdentifier: "M-1",
            points: 100,
            reason: "reward",
        }), /Not enough points/);
    ...
    it("fails when the member does not exist", async () => {
        ...
        await assert.rejects(awardPoints(missingRepo, {
