...
function isInternalSmokeMember(row: LoyaltyMemberRow) {
  ...
  return email.includes("codex-smoke") || email === "contract.member@example.com" || memberNumber.includes("test-api") || name === "codex smoke";
}
...
async function findMember(id: string) {
  ...
  if (rows[0] && !isInternalSmokeMember(rows[0])) return rows[0];
  ...
  return rows[0] && !isInternalSmokeMember(rows[0]) ? rows[0] : null;
}

async function findMembersByQuery(query: { email?: string; memberId?: string }, limit: number) {
  ...
  if (email) {
    const rows = await supabaseRest<LoyaltyMemberRow[]>(
      `loyalty_members?select=${memberSelect()}&email=ilike.${encodeURIComponent(email)}&limit=${limit}`,
    );
    return rows.filter((member) => !isInternalSmokeMember(member));
  }

  const encodedMemberId = encodeURIComponent(memberId);
  let rows = await supabaseRest<LoyaltyMemberRow[]>(
    `loyalty_members?select=${memberSelect()}&member_number=eq.${encodedMemberId}&limit=${limit}`,
  );
  rows = rows.filter((member) => !isInternalSmokeMember(member));
  if (rows.length > 0) return rows;

  if (Number.isFinite(Number(memberId))) {
    rows = await supabaseRest<LoyaltyMemberRow[]>(
      `loyalty_members?select=${memberSelect()}&id=eq.${encodeURIComponent(String(Number(memberId)))}&limit=${limit}`,
    );
    rows = rows.filter((member) => !isInternalSmokeMember(member));
    if (rows.length > 0) return rows;

    rows = await supabaseRest<LoyaltyMemberRow[]>(
      `loyalty_members?select=${memberSelect()}&member_id=eq.${encodeURIComponent(String(Number(memberId)))}&limit=${limit}`,
    );
    return rows.filter((member) => !isInternalSmokeMember(member));
  }
...
export function createServer() {
  ...
  app.get("/members", async (request) => {
    ...
    const filteredRows = rows.filter((member) => !isInternalSmokeMember(member) && matchesMember(member, query.email, query.memberId));
    return {
      ok: true,
