...
const STOP_WORDS = new Set([
  "the", "is", "at", "which", "and", "on", "in", "with", "a", "an", "of", "to",
  "for", "it", "as", "be", "this", "that", "are", "was", "but", "not", "have",
  "from", "they", "we", "you", "i", "my", "me", "so", "if", "or", "can", "will",
  "just", "about", "like", "out", "up", "down", "all", "get", "very", "would"
]);
...
function processFeedbackInsights(feedbackRows: any[]) {
  ...
  for (const row of feedbackRows) {
    const r = Number(row.rating || 5);
    if (r >= 4) sentiment.positive++;
    else if (r === 3) sentiment.neutral++;
    else sentiment.negative++;

    const text = String(row.comment || "").toLowerCase();
    const words = text.split(/[\W_]+/).filter((w: string) => w.length > 2 && !STOP_WORDS.has(w));
    
    const uniqueWords = new Set(words);
    uniqueWords.forEach((w: string) => {
      termDocFreq.set(w, (termDocFreq.get(w) || 0) + 1);
    });
  ...
  for (const words of docTokens) {
    const termFreq = new Map<string, number>();
    for (const w of words) termFreq.set(w, (termFreq.get(w) || 0) + 1);

    termFreq.forEach((freq, word) => {
      const tf = freq / (words.length || 1);
      const df = termDocFreq.get(word) || 1;
      const idf = Math.log(N / df);
      const score = tf * idf;
      tfIdfScores.set(word, (tfIdfScores.get(word) || 0) + score);
    });
  }
  ...
  const topTopics = sortedWords.slice(0, 3).map(kw => {
    let count = 0;
    for (const words of docTokens) {
      if (words.includes(kw.word)) count++;
    }
    return { topic: kw.word.charAt(0).toUpperCase() + kw.word.slice(1), count };
  });

  return {
    sentimentSplit: sentiment,
    wordCloud: top30Words,
    topTopics: topTopics
  };
}
...
export function createServer() {
  ...
  app.post("/feedback-insights/generate", async (request, reply) => {
    ...
    if (fetchError) {
      reply.code(500).send({ ok: false, error: "failed_to_fetch_feedback" });
      return;
    }

    const insights = processFeedbackInsights(rawFeedback || []);
    const { data, error } = await supabase.from("feedback_insights").insert({
    ...
    if (error) {
      if (tableMissing(error, "feedback_insights")) {
        // If table schema is slightly different or missing, return generated insights directly
        return { ok: true, insights, warning: "feedback_insights_table_missing" };
      }
      reply.code(500).send({ ok: false, error: "failed_to_save_insights" });
      return;
    }
  ...
  app.get("/feedback-insights/latest", async (request, reply) => {
    const { data, error } = await supabase
    ...
    if (error) {
      if (tableMissing(error, "feedback_insights")) return { ok: true, insights: null };
      reply.code(500).send({ ok: false, error: "failed_to_fetch_insights" });
      return;
    }
    ...
    return { 
      ok: true, 
      insights: {
        sentimentSplit: data.sentiment_split,
