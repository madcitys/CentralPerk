...
function upsertMemoryCampaign(input: CampaignInput): Campaign {
  const existingIdx = memory.campaigns.findIndex((c) => c.id === input.id || c.campaign_code === input.campaignCode);
  const record = {
    ...(existingIdx >= 0 ? memory.campaigns[existingIdx] : {}),
    ...{
      id: input.id ?? memory.campaigns[existingIdx]?.id ?? `cmp-${Date.now()}`,
...
export async function getCampaignById(campaignId: string): Promise<Campaign | null> {
  const memoryCampaign = memory.campaigns.find((item) => String(item.id) === campaignId);
  if (useMemory) return memoryCampaign ? mapCampaign(memoryCampaign) : null;
  ...
  const { data, error } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (error || !data) return memoryCampaign ? mapCampaign(memoryCampaign) : null;
  return mapCampaign(data);
}

export async function publishCampaign(campaignId: string): Promise<Campaign> {
  if (useMemory) {
    const campaign = memory.campaigns.find((item) => String(item.id) === campaignId);
...
export async function getCampaignBudgetStatus(campaignId: string) {
  ...
  if (!campaign) throw new Error("Campaign not found.");
  const performance = (await loadCampaignPerformance()).find((row: any) => performanceCampaignId(row) === campaignId);
